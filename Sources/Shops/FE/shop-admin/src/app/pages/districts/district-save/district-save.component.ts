...
export class DistrictSaveComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  isLoading = false;
  // Single model
  districtModel: IDistrictModel = { id: 0, name: '', code: '', cityId:0 };
  ...
  constructor(
    private apiService: DistrictService,
  ...
  getData(id: number) {
    this.apiService.getDistrict(id).subscribe((user: any) => {
      this.districtModel = user.value;
    });
  }

  create() {
    this.districtModel = { id: 0, name: '', code: '', cityId:0 };
  }
  ...
  onSubmit(event: Event, myForm: NgForm) {
    ...
    const updateFn = () => {
      this.apiService.updateDistrict(this.districtModel.id, this.districtModel).subscribe({
        next: () => {
    ...
    const createFn = () => {
      this.apiService.createDistrict(this.districtModel).subscribe({
        next: () => {
          this.showAlert(successAlert);
        },
        error: (error) => {
